...
const Comparacion = () => {
  ...
  const [iteraciones2, setIteraciones2] = useState([]);
  const [iteraciones1, setIteraciones1] = useState([]);
  ...
  const validar = (
    ...
    let res = true;
    switch (true) {
      case corridaParseado < 3 || corridaParseado > 1000:
        Alert.alert(
          'Error',
  ...
  const handleCalcular = () => {
    ...
    if (validar(apuestaParseado, color, corridaParseado, cantidadParseado)) {
      ...
      const iteracionesArray2 = [];
      let cantidadI2 = cantidadParseado;
      let apuestaI2 = apuestaParseado;
      let cantidadN2;
      let apuestaN2;
      ...
      for (let i = 1; i <= corridaParseado; i++) {
        ...
        if (
          (cantidadI >= apuestaI || cantidadI2 >= apuestaI2) &&
          apuestaI <= 500
        ) {
          if (auxiliar === 'VERDE') {
            if (colorA === 'VERDE') {
              cantidadN = cantidadI;
              cantidadN2 = cantidadI2;
              auxiliar = 'VERDE';
              apuestaN = apuestaI;
              apuestaN2 = apuestaI2;
            } else {
              ...
              cantidadN2 =
                colorA === color ? cantidadI2 : cantidadI2 - apuestaI2;
              apuestaN2 = colorA === color ? apuestaParseado : apuestaI2 * 2;
            }
          } else {
            if (colorA === 'VERDE') {
              cantidadN = cantidadI;
        ...
        cantidadI2 = parseFloat(cantidadN2);
        apuestaI2 = parseFloat(apuestaN2);
        iteracionesArray2.push(iteracion2);

        colorsArray.push(colorA);
      }
      setIteraciones1(iteracionesArray);
      setCantidadFinal1(cantidadI);
